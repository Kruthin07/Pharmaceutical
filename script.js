...
row.className = "form-row sale-line"; // tag as top-level sale row
row.innerHTML = `
...
// sales rows container: ensure at least one true sale-line exists
...
// Record ALL sales from rows (fixed)
function recordAllSales() {
const rows = Array.from(document.querySelectorAll("#saleRows .sale-line")); // only top-level rows
...
renderAll();
showView("sales"); // ensure you see the log update immediately
