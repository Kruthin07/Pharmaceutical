      ...
      let state = {
        medicines: [],   // {id,name,batch,supplierId,qty,expiry (ISO),rating,price,controlled?}
        suppliers: [],   // {id,name,contact,notes}
        sales: [],       // {id,date,medId,qty,total,unitPrice?}
        purchases: [],   // {date,supplierId,items:[{medId,batch,qty,price}]}
        disposals: [],   // {date,medId,batch,qty,reason}
        controlledLogs: [], // {date,medId,batch,qty,type,note}
        gstInvoices: [], // {date,invoiceNo,items:[{medId,batch,qty,price,gstPercent}]}
      ...
      function seedSample() {
        ...
        const medicines = names.map((name, i) => {
          ...
          return {
            ...
            controlled: Math.random() < 0.08 // ~8% as controlled for demo
          };
        });
        ...
        for (let i = 0; i < transactions; i++) {
          ...
          sales.push({ id: id("t"), date: date.toISOString(), medId: med.id, qty, total, unitPrice: med.price });
          if (med.controlled) {
            state.controlledLogs.push({
              date: date.toISOString(), medId: med.id, batch: med.batch, qty, type: 'sale', note: 'Retail sale'
            });
          }
        }
        ...
        state = {
          ...
          purchases: [],
          disposals: [],
          controlledLogs: state.controlledLogs || [],
          gstInvoices: [],
      ...
      function renderMedTable() {
        ...
        state.medicines
          ...
          .forEach((m) => {
            ...
            tr.innerHTML = `
              <td>${m.name}${m.controlled ? ' <span class="badge" title="Controlled drug">CD</span>' : ''}</td>
              <td>${m.batch}</td>
      ...
      function openEditMed(id) {
        ...
        const ctrl = prompt("Controlled drug? (y/n)", m.controlled ? "y" : "n");
        if (ctrl != null) m.controlled = /^y/i.test(ctrl);
        saveState();
        renderAll();
      }
      ...
      function addNewMedicine() {
        ...
        const controlled = /^y/i.test(prompt("Is this a controlled drug? (y/n)", "n") || "n");
        const med = { id: id("m"), name, batch, supplierId: supplier || "", qty, expiry: expiry || "", rating, price, controlled };
      ...
      function restockMed(medId, suggested) {
        ...
        // Track as purchase entry (simple)
        state.purchases.push({
          date: new Date().toISOString(),
          supplierId: med.supplierId || "",
          items: [{ medId: med.id, batch: med.batch, qty, price: med.price || 0 }]
        });
      ...
      function disposeMedicine(medId) {
        ...
        saveState();
        renderAll();
        alert(`Disposed ${qty} units of ${med.name}.`);
      }

      // Record ALL sales from rows (with controlled & logs)
      function recordAllSales() {
        ...
        for (const row of rows) {
          ...
          state.sales.push({ id: id("t"), date: new Date().toISOString(), medId, qty, total, unitPrice });

          if (med.controlled) {
            state.controlledLogs.push({
              date: new Date().toISOString(),
              medId, batch: med.batch, qty, type: 'sale', note: 'Retail sale'
            });
          }
          totalLines++;
        }
      ...
      function inRangeAudit(iso, fromIso, toIso) {
        if (!fromIso && !toIso) return true;
        const t = new Date(iso).getTime();
        return (!fromIso || t >= new Date(fromIso).getTime()) &&
               (!toIso   || t <= new Date(toIso).getTime());
      }
      function buildAuditData() {
        ...
        const idxMed = Object.fromEntries(data.medicines.map(m => [m.id + '|' + m.batch, m]));
        ...
        const purchases = (data.purchases || []).filter(p => inRangeAudit(p.date, from, to))
          .flatMap(p => p.items.map(it => ({
            date: p.date,
            supplier: idxSup[p.supplierId]?.name || p.supplierId || '',
            med: (idxMed[it.medId + '|' + it.batch]?.name) || idxMedById[it.medId]?.name || it.medId,
            batch: it.batch,
            qty: it.qty,
            price: it.price,
            amount: +(it.qty * it.price).toFixed(2)
          })));
        ...
        const sales = (data.sales || []).filter(s => inRangeAudit(s.date, from, to))
          .map(s => {
            ...
            return {
              date: s.date,
              med: med?.name || s.medId,
              batch: med?.batch || '',
              qty: s.qty,
              unitPrice: (s.unitPrice ?? med?.price ?? 0),
              amount
            };
          });
        ...
        const disposals = (data.disposals || []).filter(d => inRangeAudit(d.date, from, to))
          .map(d => {
            const med = idxMed[d.medId + '|' + d.batch] || idxMedById[d.medId];
            return { date: d.date, med: med?.name || d.medId, batch: d.batch, qty: d.qty, reason: d.reason };
          });

        const controlled = (data.controlledLogs || []).filter(c => inRangeAudit(c.date, from, to))
          .map(c => {
            const med = idxMed[c.medId + '|' + c.batch] || idxMedById[c.medId];
            return { date: c.date, med: med?.name || c.medId, batch: c.batch, qty: c.qty, type: c.type, note: c.note || '' };
          });

        const gst = (data.gstInvoices || []).filter(i => inRangeAudit(i.date, from, to))
          .map(i => {
            const total = i.items.reduce((sum, it) => {
              const med = idxMed[it.medId + '|' + it.batch] || idxMedById[it.medId];
              const price = it.price ?? med?.price ?? 0;
              const line = it.qty * price;
              const tax = line * (Number(it.gstPercent || 0) / 100);
              return sum + line + tax;
            }, 0);
            return { date: i.date, invoiceNo: i.invoiceNo, items: i.items.length, total: +total.toFixed(2) };
          });

        return {
          purchases, sales, disposals, controlled, gst,
          meta: {
            generatedAt: new Date().toISOString(),
            rangeFrom: from || 'All', rangeTo: to || 'All',
            version: 'audit.v1'
          }
        };
      }
      ...
      async function exportAuditPDF() {
        ...
        function section(title, rows, fields, maxRows = 20) {
        ...
        section('Sales',      audit.sales,     ['date','med','batch','qty','unitPrice','amount']);
        section('Disposals',  audit.disposals, ['date','med','batch','qty','reason']);
        section('Controlled', audit.controlled,['date','med','batch','qty','type','note']);
        section('GST',        audit.gst,       ['date','invoiceNo','items','total']);

        if (y > 770) { doc.addPage(); y = 40; }
        doc.setFontSize(8);
        line(`Signature (optional): See signed JSON bundle for verification.`, y); y += 12;
      ...
      // ---------- Cloud Backup (Firebase compat, optional) ----------
      ...
      } catch(e){ /* noop if Firebase not configured */ }
      ...
      async function backupToCloud() {
        try {
          ...
          alert('Cloud backup failed. Configure Firebase keys in the code.');
        }
      }
      ...
      document.getElementById("resetData").addEventListener("click", () => {
        if (confirm("Reset all data?")) {
          state = { medicines: [], suppliers: [], sales: [], purchases: [], disposals: [], controlledLogs: [], gstInvoices: [], settings: { lowStockThreshold: 30 } };
          saveState();
          renderAll();
        }
      });
      ...
      function exportAuditExcel() {
  try {
    // helper: nice date for filenames
    ...
    // Build the "ALL sales" sheet (ignores report range)
    const allSalesRows = state.sales
      .slice()                      // don’t mutate
    ...
    // Also keep your range-based audit (uses your existing buildAuditData)
    const { purchases, sales, disposals, controlled, gst, meta } = buildAuditData();
    ...
    // 1) Full sales (new, always included)
    addSheet("Sales_All", allSalesRows, [22, 28, 14, 22, 8, 14, 14]);

    // 2) Range-based sheets (kept from your audit)
    addSheet("Sales_Range", sales,     [22, 28, 14, 8, 14, 14]);
    addSheet("Purchases",   purchases, [22, 22, 28, 14, 10, 12]);
    addSheet("Disposals",   disposals, [22, 28, 14, 8, 16]);
    addSheet("Controlled",  controlled,[22, 28, 14, 8, 10, 20]);
    addSheet("GST",         gst,       [22, 18, 8, 14]);
    addSheet("Meta",        [meta],    [30, 20, 20, 14]);

    // Write as ArrayBuffer and force download
    ...
    const a = document.createElement("a");
    a.href = URL.createObjectURL(blob);
    a.download = `Audit_with_All_Sales_${safeDate()}.xlsx`;
